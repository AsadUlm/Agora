...
class GroqProvider(LLMService):
    ...
    async def generate(self, request: LLMRequest) -> LLMResponse:
        ...
        try:
            ...
            safe = classify_provider_error(exc, provider="groq", model=model)
            err = LLMGenerationError(f"Groq API timeout: {exc}")
            ...
            err = LLMGenerationError(f"Groq API error {exc.status_code}: {exc.message}")
            err.safe_error = safe  # type: ignore[attr-defined]
            raise err from exc
        except Exception as exc:
            safe = classify_provider_error(exc, provider="groq", model=model)
            err = LLMGenerationError(f"Groq unexpected error: {exc}")
            err.safe_error = safe  # type: ignore[attr-defined]
            raise err from exc
